...
/*********************************************获取会话策略下的ip-ports映射************************************************/
type VehicleSingleFlowStrategyItems struct {
	gorm.Model
...
func GetVehicleFStrategyItems(query string, args ...interface{}) ([]*VehicleSingleFlowStrategyItems, error) {
	...
	vehicleFStrategyItems := []*VehicleSingleFlowStrategyItems{}
	err = vgorm.Debug().
		Table("fstrategy_vehicle_items").
		Select("fstrategy_vehicle_items.*,fstrategy_items.dst_ip,fstrategy_items.dst_port").
		Where(query, args...).
		Joins("inner join fstrategy_items ON fstrategy_vehicle_items.fstrategy_item_id = fstrategy_items.fstrategy_item_id").
		Scan(&vehicleFStrategyItems).
		Error
	return vehicleFStrategyItems, err
}
