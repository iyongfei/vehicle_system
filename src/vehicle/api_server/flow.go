...
func GetFlows(c *gin.Context)  {
	...
	//flowObj:= &model.WhiteList{}
	//modelBase := model_base.ModelBaseImpl(flowObj)
	//err,recordNotFound:=modelBase.GetModelByCondition("vehicle_id = ?",[]interface{}{vehicleId}...)
	//
	//if err!=nil{
	//	logger.Logger.Error("%s vehicleId:%s,getFlows err:%s",util.RunFuncName(),vehicleId,err)
	//	logger.Logger.Print("%s vehicleId:%s,getFlows err:%s",util.RunFuncName(),vehicleId,err)
	//	ret:=response.StructResponseObj(response.VStatusServerError,response.ReqGetFlowFailMsg,"")
	//	c.JSON(http.StatusOK,ret)
	//	return
	//}
	//
	//if recordNotFound{
	//	logger.Logger.Error("%s vehicleId:%s,recordNotFound",util.RunFuncName(),vehicleId)
	//	logger.Logger.Print("%s vehicleId:%s,recordNotFound",util.RunFuncName(),vehicleId)
